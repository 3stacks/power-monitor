...
function checkUnclosedOutages() {
    ...
    if (unclosed) {
        ...
        db.prepare("UPDATE outages SET ended_at = ?, duration_minutes = ?, notes = ? WHERE id = ?").run(now, durationMinutes, "Closed on startup (server may have lost power)", unclosed.id);
        console.log(`[STARTUP] Closed unclosed outage #${unclosed.id} - duration: ${durationMinutes} minutes (approximate)`);
    }
}
// Ping function
async function ping() {
...
// Record ping result
function recordPing(success) {
    const timestamp = new Date().toISOString();
    db.prepare("INSERT INTO pings (timestamp, success) VALUES (?, ?)").run(timestamp, success ? 1 : 0);
    // Keep only last 24 hours of pings
    db.prepare("DELETE FROM pings WHERE timestamp < datetime('now', '-1 day')").run();
}
// Start an outage
function startOutage() {
    ...
    const result = db
        ...
        .run(startedAt);
    currentOutageId = result.lastInsertRowid;
...
function endOutage() {
    ...
    const durationMinutes = Math.round((new Date(endedAt).getTime() - startedAt.getTime()) / (1000 * 60));
    db.prepare("UPDATE outages SET ended_at = ?, duration_minutes = ? WHERE id = ?").run(endedAt, durationMinutes, currentOutageId);
    console.log(`[OUTAGE ENDED] ${endedAt} - Outage #${currentOutageId} lasted ${durationMinutes} minutes`);
    // Freezer warning
...
function showHistory() {
    const outages = db
        .prepare("SELECT * FROM outages ORDER BY started_at DESC LIMIT 20")
        .all();
...
function generateStatusPage() {
    ...
    const dayBars = uptimeData.map((day) => {
        ...
        if (day.uptimePercent < 100)
            barColor = "#f59e0b"; // yellow
        if (day.uptimePercent < 95)
            barColor = "#ef4444"; // red
        return `
      <div class="day-bar">
    ...
    const outageRows = outages.length === 0
        ...
        : outages.map((o) => {
            ...
            const notes = o.notes || "";
            let rowClass = "";
            if (o.duration_minutes && o.duration_minutes >= 240)
                rowClass = "critical";
            else if (o.duration_minutes && o.duration_minutes >= 60)
                rowClass = "warning";
            else if (!o.ended_at)
                rowClass = "ongoing";
            return `
